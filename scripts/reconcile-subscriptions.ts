...
import { stripeSubscriptionPeriodEnd, toSubscriptionStatus } from "../src/lib/subscription";
  ...
  for (const store of stores) {
    try {
      const subs = await stripe.subscriptions.list({
        customer: store.stripeCustomerId as string,
        status: "all",
        limit: 1,
      });
      ...
      console.log(
        `sync  ${store.name} → ${toSubscriptionStatus(sub.status)} end=${periodEnd?.toISOString() ?? "—"} (${sub.id})`,
      );
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      console.error(`fail  ${store.name}: ${message}`);
    }
  }
}
